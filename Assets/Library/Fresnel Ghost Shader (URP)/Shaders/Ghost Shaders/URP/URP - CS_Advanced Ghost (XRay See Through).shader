Shader "Ciconia Studio/CS_Ghost/URP/CS_Advanced Ghost (XRay See Through)"
{
	Properties
	{
		...
		[Space(15)][Header(XRay Properties___________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________))][Space(15)] _XRayColor( "Color -->(Transparency A)", Color ) = ( 0, 1, 0.980212, 0.1019608 )
	...
	LOD 0
		...
		Pass
		{
			...
			half4 CalculateShadowMask343_g7(  )
			{
			...
			) : SV_Target
			{
				...
				float3 unpack571 = UnpackNormalScale( tex2D( _BumpMap, uv_BumpMap ), _BumpScale );
				unpack571.z = lerp( 1, unpack571.z, saturate(_BumpScale) );
				float3 NormalmapXYZ572 = unpack571;
				...
				float3 WorldPosition288_g7 = PositionWS;
				float3 WorldPosition337_g7 = WorldPosition288_g7;
				float2 ScreenUV286_g7 = (ScreenPosNorm).xy;
				float2 ScreenUV337_g7 = ScreenUV286_g7;
				float3 tanNormal280_g7 = NormalmapXYZ572;
				float3 worldNormal280_g7 = normalize( float3( dot( tanToWorld0, tanNormal280_g7 ), dot( tanToWorld1, tanNormal280_g7 ), dot( tanToWorld2, tanNormal280_g7 ) ) );
				float3 WorldNormal281_g7 = worldNormal280_g7;
				float3 WorldNormal337_g7 = WorldNormal281_g7;
				half4 localCalculateShadowMask343_g7 = CalculateShadowMask343_g7();
				float4 ShadowMask360_g7 = localCalculateShadowMask343_g7;
				float4 ShadowMask337_g7 = ShadowMask360_g7;
				float3 localAdditionalLightsLambertMask171x337_g7 = AdditionalLightsLambertMask171x( WorldPosition337_g7 , ScreenUV337_g7 , WorldNormal337_g7 , ShadowMask337_g7 );
				float3 tanNormal585 = NormalmapXYZ572;
				float3 bakedGI585 = ASEIndirectDiffuse( input, float3( dot( tanToWorld0, tanNormal585 ), dot( tanToWorld1, tanNormal585 ), dot( tanToWorld2, tanNormal585 ) ), PositionWS, ViewDirWS );
				MixRealtimeAndBakedGI( ase_mainLight, float3( dot( tanToWorld0, tanNormal585 ), dot( tanToWorld1, tanNormal585 ), dot( tanToWorld2, tanNormal585 ) ), bakedGI585, half4( 0, 0, 0, 0 ) );
				float3 tanNormal594 = NormalmapXYZ572;
				half3 reflectVector594 = reflect( -ViewDirWS, float3( dot( tanToWorld0, tanNormal594 ), dot( tanToWorld1, tanNormal594 ), dot( tanToWorld2, tanNormal594 ) ) );
				float3 indirectSpecular594 = GlossyEnvironmentReflection( reflectVector594, PositionWS, 1.0 - _Glossiness, 1.0, ScreenPosNorm.xy );
				float3 tanNormal584 = NormalmapXYZ572;
				float fresnelNdotV584 = dot( float3( dot( tanToWorld0, tanNormal584 ), dot( tanToWorld1, tanNormal584 ), dot( tanToWorld2, tanNormal584 ) ), ViewDirWS );
				float fresnelNode584 = ( 0.04 + 1.0 * pow( max( 1.0 - fresnelNdotV584 , 0.0001 ), 1.11 ) );
				...
				#endif
				float4 lerpResult595 = lerp( ( ( _Color * tex2D( _MainTex, uv_MainTex ) ) * float4( ( ( ( max( dotResult577, 0.0 ) * ( ase_lightAtten * ase_lightColor.rgb ) ) + localAdditionalLightsLambertMask171x337_g7 ) + bakedGI585 ) , 0.0 ) ) , float4( indirectSpecular594 , 0.0 ) , staticSwitch592);
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float3 temp_output_272_0 = ( temp_output_271_0 + NormalmapXYZ572 );
				float TranslationSpeed329 = _TranslationSpeed;
				float2 temp_cast_5 = (TranslationSpeed329).xx;
				float2 uv_DetailMap = input.ase_texcoord3.xy * _DetailMap_ST.xy + _DetailMap_ST.zw;
				float TexturesScale451 = _TexturesScale;
				float4 unityObjectToClipPos420 = TransformWorldToHClip( TransformObjectToWorld( ( input.ase_texcoord4.xyz ).xyz ) );
				float4 computeScreenPos421 = ComputeScreenPos( unityObjectToClipPos420 );
				float4 unityObjectToClipPos437 = TransformWorldToHClip( TransformObjectToWorld( ( float3( 0, 0, 0 ) ).xyz ) );
				float4 computeScreenPos438 = ComputeScreenPos( unityObjectToClipPos437 );
				float4 transform428 = mul(GetObjectToWorldMatrix(),float4( PositionWS , 0.0 ));
				...
				float sin301 = sin( ( mulTime304 + radians( RotationAngle331 ) ) );
				float2 rotator301 = mul( staticSwitch394.xy - float2( 0.5,0.5 ) , float2x2( cos301 , -sin301 , sin301 , cos301 )) + float2( 0.5,0.5 );
				float2 panner282 = ( ( _TimeParameters.x * 0.05 ) * temp_cast_5 + rotator301);
				float4 tex2DNode295 = tex2D( _DetailMap, panner282 );
				float2 temp_cast_10 = (-TranslationSpeed329).xx;
				float mulTime317 = _TimeParameters.x * -RotationSpeed330;
				float cos322 = cos( ( mulTime317 + radians( -RotationAngle331 ) ) );
				float sin322 = sin( ( mulTime317 + radians( -RotationAngle331 ) ) );
				float2 rotator322 = mul( staticSwitch394.xy - float2( 0.5,0.5 ) , float2x2( cos322 , -sin322 , sin322 , cos322 )) + float2( 0.5,0.5 );
				float2 panner325 = ( ( _TimeParameters.x * 0.05 ) * temp_cast_10 + rotator322);
				#ifdef _DUPLICATEDETAILS_ON
				float4 staticSwitch341 = ( CalculateContrast(_ContrastDetailMap,( tex2D( _DetailMap, ( 1.0 - panner325 ) ) + tex2DNode295 )) + _SpreadDetailMap );
				#else
				float4 staticSwitch341 = ( CalculateContrast(( _ContrastDetailMap * 1.25 ),tex2DNode295) + _SpreadDetailMap );
				...
				float fresnelNdotV536 = dot( NormalWS, ViewDirWS );
				float fresnelNode536 = ( _XRayBias + clampResult559 * pow( max( 1.0 - fresnelNdotV536 , 0.0001 ), clampResult560 ) );
