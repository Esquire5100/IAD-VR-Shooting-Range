Shader "Ciconia Studio/CS_Ghost/URP/CS_Advanced Ghost"
{
	...
	LOD 0
		...
		Pass
		{
			...
			half4 CalculateShadowMask343_g3(  )
			{
						 ...
						 ) : SV_Target
			{
				...
				float3 WorldPosition288_g3 = PositionWS;
				float3 WorldPosition337_g3 = WorldPosition288_g3;
				float2 ScreenUV286_g3 = (ScreenPosNorm).xy;
				float2 ScreenUV337_g3 = ScreenUV286_g3;
				float3 tanNormal280_g3 = NormalmapXYZ148;
				float3 worldNormal280_g3 = normalize( float3( dot( tanToWorld0, tanNormal280_g3 ), dot( tanToWorld1, tanNormal280_g3 ), dot( tanToWorld2, tanNormal280_g3 ) ) );
				float3 WorldNormal281_g3 = worldNormal280_g3;
				float3 WorldNormal337_g3 = WorldNormal281_g3;
				half4 localCalculateShadowMask343_g3 = CalculateShadowMask343_g3();
				float4 ShadowMask360_g3 = localCalculateShadowMask343_g3;
				float4 ShadowMask337_g3 = ShadowMask360_g3;
				float3 localAdditionalLightsLambertMask171x337_g3 = AdditionalLightsLambertMask171x( WorldPosition337_g3 , ScreenUV337_g3 , WorldNormal337_g3 , ShadowMask337_g3 );
				float3 tanNormal576 = NormalmapXYZ148;
				float3 bakedGI576 = ASEIndirectDiffuse( input, float3( dot( tanToWorld0, tanNormal576 ), dot( tanToWorld1, tanNormal576 ), dot( tanToWorld2, tanNormal576 ) ), PositionWS, ViewDirWS );
				...
				half3 reflectVector574 = reflect( -ViewDirWS, float3( dot( tanToWorld0, tanNormal574 ), dot( tanToWorld1, tanNormal574 ), dot( tanToWorld2, tanNormal574 ) ) );
				float3 indirectSpecular574 = GlossyEnvironmentReflection( reflectVector574, PositionWS, 1.0 - _Glossiness, 1.0, ScreenPosNorm.xy );
				float3 tanNormal581 = NormalmapXYZ148;
				float fresnelNdotV581 = dot( float3( dot( tanToWorld0, tanNormal581 ), dot( tanToWorld1, tanNormal581 ), dot( tanToWorld2, tanNormal581 ) ), ViewDirWS );
				...
				#endif
				float4 lerpResult585 = lerp( ( ( _Color * tex2D( _MainTex, uv_MainTex ) ) * float4( ( ( ( max( dotResult596, 0.0 ) * ( ase_lightAtten * ase_lightColor.rgb ) ) + localAdditionalLightsLambertMask171x337_g3 ) + bakedGI576 ) , 0.0 ) ) , float4( indirectSpecular574 , 0.0 ) , staticSwitch240);
				float4 CustomLight96 = lerpResult585;
				
				float clampResult2_g4 = clamp( _FresnelTransIntensity , 0.0 , 400.0 );
				float fresnelNdotV5_g4 = dot( NormalWS, ViewDirWS );
				float fresnelNode5_g4 = ( _FresnelTransBias + clampResult2_g4 * pow( max( 1.0 - fresnelNdotV5_g4 , 0.0001 ), _FresnelTransPower ) );
				float temp_output_7_0_g4 = saturate( ( 1.0 - fresnelNode5_g4 ) );
				float Opacity250 = (  (0.5 + ( ( 1.0 - _ShadowOpacity ) - 0.0 ) * ( 1.0 - 0.5 ) / ( 1.0 - 0.0 ) ) * temp_output_7_0_g4 );
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float clampResult2_g4 = clamp( _FresnelTransIntensity , 0.0 , 400.0 );
				float fresnelNdotV5_g4 = dot( ase_normalWS, ase_viewDirWS );
				float fresnelNode5_g4 = ( _FresnelTransBias + clampResult2_g4 * pow( max( 1.0 - fresnelNdotV5_g4 , 0.0001 ), _FresnelTransPower ) );
				float temp_output_7_0_g4 = saturate( ( 1.0 - fresnelNode5_g4 ) );
				float Opacity250 = (  (0.5 + ( ( 1.0 - _ShadowOpacity ) - 0.0 ) * ( 1.0 - 0.5 ) / ( 1.0 - 0.0 ) ) * temp_output_7_0_g4 );
		...
		Pass
		{
						 ...
						 )
			{
				...
				float3 ase_viewDirWS = normalize( ase_viewVectorWS );
				float clampResult2_g4 = clamp( _FresnelTransIntensity , 0.0 , 400.0 );
				float fresnelNdotV5_g4 = dot( NormalWS, ase_viewDirWS );
				float fresnelNode5_g4 = ( _FresnelTransBias + clampResult2_g4 * pow( max( 1.0 - fresnelNdotV5_g4 , 0.0001 ), _FresnelTransPower ) );
				float temp_output_7_0_g4 = saturate( ( 1.0 - fresnelNode5_g4 ) );
				float Opacity250 = (  (0.5 + ( ( 1.0 - _ShadowOpacity ) - 0.0 ) * ( 1.0 - 0.5 ) / ( 1.0 - 0.0 ) ) * temp_output_7_0_g4 );
